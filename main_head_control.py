...
class HeadControl(Node):
    ...
    def __init__(self):
        ...
        self.create_subscription(
            String,
            '/pid_params',
            self._pid_params_callback,
            10
        )
        self.get_logger().info("PID Tuner: subscriber /pid_params aktif")
    ...
    def _pid_params_callback(self, msg):
        """
        Terima nilai PID dari GUI dan update PIDControl secara langsung.
        ...
        try:
            ...
            if pan:
                ...
                self.pid[HEAD_PAN].Init()   # reset akumulator setelah ubah gains
                self.get_logger().info(
                    f"[PID PAN]  Kp={pan.get('kp')} Ki={pan.get('ki')} Kd={pan.get('kd')}"
                )
            ...
            if tilt:
                ...
                self.get_logger().info(
                    f"[PID TILT] Kp={tilt.get('kp')} Ki={tilt.get('ki')} Kd={tilt.get('kd')}"
                )
    
        except (json.JSONDecodeError, KeyError, ValueError) as e:
            self.get_logger().warn(f"[PID Tuner] Gagal parse pesan: {e}")
